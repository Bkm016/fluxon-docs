...
dependencies {
    ...
    implementation("org.tabooproject.fluxon:fluxon-core-jsr223:1.4.0")
}
```
...
JSR223 引擎底层仍使用 `FluxonRuntime.getInstance().newEnvironment()` 创建环境，因此你通过 `FluxonRuntime` 注册的系统函数/扩展函数也会对 JSR223 引擎生效。宿主注册方式见：
