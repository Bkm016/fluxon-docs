...
`Environment` 是 Fluxon 执行期的“作用域 + 符号表”。解释器与编译执行引擎都会基于同一个 `Environment` 完成变量读写、函数调用与扩展函数分派（`target::method(...)`）。
...
<Note>`Environment` 通常通过 `FluxonRuntime#getInstance().newEnvironment()` 创建。注册新的函数/扩展/变量后，重新创建环境以刷新运行时缓存。</Note>
...
命名空间函数（如 `fs:time`）需要脚本先 `import 'fs:time'` 才会被解析到（见 `appendix/glossary`）。
