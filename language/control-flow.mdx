...
在 Fluxon 中，控制结构本身也是表达式。大多数分支与循环形式都能够产生值，并参与更大表达式的求值。本章介绍 `if`、`when`、循环、`break` / `continue` 以及 `try` 相关语法。
...
Fluxon 提供 `throw` 与 `try` / `catch` / `finally` 结构来处理运行时错误。和其他控制结构一样，`try` 是一个表达式，可以产生值。
...
- `language/variables-and-references`
- `language/operators-and-priority`
