...
`when` 支持使用 `in` 和 `!in` 检查值是否在集合、映射或区间内：
...
- `in <collection>` 检查主题值是否在集合中
- `!in <collection>` 检查主题值是否不在集合中
- 支持列表、映射、区间等可迭代值

### 类型匹配

`when` 支持使用 `is` 运算符进行类型检查：
...
- 使用 `is <TypeName>` 进行运行时类型检查
- 支持类型别名（不区分大小写）：`string`、`int`、`list`、`map` 等
- 支持完全限定类名：`is java.util.ArrayList`
- `null` 值不匹配任何类型，会进入 `else` 分支
- 可与其他匹配类型混合使用：
...
详见 [运算符与优先级 - 类型检查](/language/operators-and-priority#类型检查运算符-is)。
