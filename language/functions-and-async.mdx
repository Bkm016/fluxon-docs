...
在 Fluxon 中，**函数** 用于封装可复用的计算逻辑。运行时还提供两类“调度语义”：
...
<Note>主线程执行器由宿主通过 `FluxonRuntime#setPrimaryThreadExecutor` 提供；详见 `developer/runtime-config`。</Note>

### 等待与结果获取

`await expr` 会在 `expr` 返回 `Future/CompletableFuture` 时等待其完成，否则直接返回 `expr` 的值。它是一个“可等待也可直通”的表达式，因此可以自然嵌入控制流与函数返回。
...
- `developer/runtime-registry`（宿主注册函数/异步函数）
- `developer/extension-api`（扩展函数与上下文调用）
