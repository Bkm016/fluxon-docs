...
运行时仅暴露少量注解用于描述导出方法的语义，主要位于 `core/src/main/java/org/tabooproject/fluxon/runtime/java`，由 `ExportRegistry` 扫描并生成扩展函数（见 `developer/export-bridge`）。
...
- 类型校验由桥接器在调用前统一执行（`Intrinsics.checkArgumentTypes(...)`），错误类型与手写扩展保持一致（如 `ArgumentTypeMismatchError`）。
