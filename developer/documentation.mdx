...
- **术语统一**：新增术语时同步维护 `appendix/glossary`，避免同一概念多种叫法。
...
2. **刷新函数目录（如适用）**：
   ...
   - 如涉及 VS Code 扩展补全，同步目录（参考 `tooling/vscode`）。
3. **更新文档（按影响面选择）**：
   - 语法/语义：`language/*`
   - 执行模型/嵌入：`runtime/*`
   - 宿主注册/导出/扩展：`developer/*`
   - 只影响使用路径：`guides/*`
4. **检查 canonical 页面**：涉及“定义/字段/清单”的内容只在 canonical 页面维护（例如函数目录见 `appendix/function-catalog`），其他页面只链接引用。
5. **本地预览**：在 `mintlify-docs` 目录运行 `pnpm exec mint dev`，确保导航可达、无 404、示例展示正常。

## 发布前检查（与版本强相关）

- `gradle.properties` 的 `version` 更新后：
  - `guides/quickstart` 中的依赖坐标同步更新。
  - `developer/release` 中的检查清单同步更新。
- 若运行时函数/扩展变化：
  - `appendix/function-catalog`（canonical）与相关引用页保持一致。
  - VS Code 扩展的函数目录同步无遗漏（参考 `tooling/vscode`）。
