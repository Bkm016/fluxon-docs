...
`@Export` 用于把 Java 方法桥接为 Fluxon 扩展函数（`target::method(...)`），避免手写 `registerExtensionFunction(...)` 的样板代码。底层由 `ExportRegistry` 扫描方法并生成 `ClassBridge`（字节码桥接器），实现“无反射调用 + 参数类型校验”。

核心类位于 `core/src/main/java/org/tabooproject/fluxon/runtime/java`：`ExportRegistry`, `ExportMethod`, `ClassBridge`, `ExportBytecodeGenerator`。
