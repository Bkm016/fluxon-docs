...
按“解析 → 分派 → 数据 → 并发”分层排查，直接对号入座后执行对应清单。

| 阶段 | 典型症状 | 立即检查 |
| --- | --- | --- |
| 解析 | `MultipleParseException` | `ex.formatDiagnostic()` 打印源码摘录；REPL 语句前加 `$` 查看 AST |
| 分派 | `FunctionNotFoundError` / `EvaluatorNotFoundError` | `Environment#getRootFunctions()`/`getRootExtensionFunctions()`；命名空间与 target 类型是否匹配 |
| 数据 | `ArgumentTypeMismatchError` / `VariableNotFoundError` / `IndexAccessError` / `VoidError` | `FunctionContext#toString()`、`availableVariables`、`IndexErrorType` |
| 并发 | 主线程函数跑在后台 / 异步任务堆积 | `registerPrimarySyncFunction` 用法、`ThreadPoolManager#getActiveTaskCount()`、执行器配置 |
...
新增语法未提供求值器导致。检查是否在 `runtime.function`/`index` 注册 evaluator；如果语法应被禁止，在解析阶段直接抛 `ParseException`，避免运行时才失败。
...
- 主线程函数：仅通过 `registerPrimarySyncFunction` / `registerSyncExtensionFunction` 标记；并使用 `FluxonRuntime#setPrimaryThreadExecutor` 指向宿主主线程。
- 异步任务：`ThreadPoolManager#getActiveTaskCount()` 观察堆积；出现 `CallerRunsPolicy` 表示队列满，新任务会落到调用线程，需要拆批或扩容。
